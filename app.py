...
# 1. CSS - Navy & Yellow Theme
st.markdown("""<style>
    .main-title { color: #1e3a8a; font-weight: 800; text-align: center; }
    .mic-icon { color: #facc15; font-size: 50px; text-align: center; }
    .stButton button {
...
VENUES = ["Underground", "The Comedy Shop", "Don't Tell", "College Gig", "Dive Bar", "Upscale Bar", "Showcase", "Open Mic", "Brewery", "Theater", "House Party", "Corporate", "Elks Club"]
...
with st.sidebar:
    ...
    if "last_res" in st.session_state:
        st.download_button("💾 Download", st.session_state["last_res"], "set.txt")
    st.markdown(f'<a href="https://www.paypal.com/paypalme/mrcoward" target="_blank"><div style="background-color: #facc15; color: #1e3a8a; text-align: center; padding: 10px; border-radius: 8px; font-weight: bold;">☕ Donate</div></a>', unsafe_allow_html=True)
...
if st.button("🚀 RUN SIMULATION", use_container_width=True):
    if city and sel_v:
        # Shortened list to prevent line-break errors
...
# 6. DISPLAY
if "last_res" in st.session_state:
    st.markdown(f"<div class='response-card'><h3>🎭 The Crowd:</h3>{st.session_state['last_res']}</div>", unsafe_allow_html=True)
